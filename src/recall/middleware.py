"""Deep Agents / LangChain AgentMiddleware for content-addressed subagent result caching."""
...
class SubagentResultCacheMiddleware(AgentMiddleware):
    """Intercept ``task`` tool calls for pure subagent profiles and serve/store cache hits.

    Compatible with LangChain ``AgentMiddleware`` / Deep Agents ``create_deep_agent``
    (verified against ``deepagents>=0.7`` and ``langchain>=1.3``):

    - Tool name/args come from ``request.tool_call["name"]`` / ``["args"]``.
    - Cache hits return a ``ToolMessage`` (not a bare string).
    - Collection bootstrap runs in ``abefore_agent``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if profiles is None:
            raise ValueError("profiles is required")
    ...
    async def awrap_tool_call(
        ...
        result_text = _message_content_to_str(result.content)
