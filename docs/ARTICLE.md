...
> **Implementation status.** The package described here ships in `src/recall/`. For install, wiring, and the map-reduce demo, start with [`SETUP.md`](SETUP.md). Live LangChain / Deep Agents APIs differ slightly from early sketch snippets below (`request.tool_call[...]`, cache hits return `ToolMessage`, bootstrap via `abefore_agent`, and live `task` often returns a `Command`) — the code in `src/recall/` is the source of truth.
